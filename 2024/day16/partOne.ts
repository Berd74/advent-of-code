...
class Player {
  ...
  clone(){
    return new Player(this.map, this.pos, this.direction, this.score);
  }

  move() {
    if (!this.alive) {return []}
    const notFilteredPossibleFields = this.map.getNextPossibleFields(this.pos)
    const possibleFields = notFilteredPossibleFields.filter(pos => pos.field.visitedPoints > this.score)
