...
export async function partOne2(path: string) {
  ...
  map.grid.forEach((row, y) => {
    row.forEach((field, x) => {
      const el = map.getElements([field.pos])[0]
      if (el?.type === 'box') {
...
class Bot {
  ...
  isMovePossible(dir: Direction, positions = [this.pos.clone()], boxes: Element[] = []):
    ...
    const newPositions = newBoxes.map(b => b.positions).flat().map(p => p.clone())
  ...
  constructor(texts: string[]) {
    ...
    this.grid = mapDesc.map((row, y) => {
      return row.split('').map((character, x) => {
        ...
        if (character === '#') {
          this.elements.push(new Element('wall', [new Pos(y, x), new Pos(y, x)], this))
        }
        if (character === 'O') {
          this.elements.push(new Element('box', [new Pos(y, x), new Pos(y, x)], this))
        }
        return new Field(
          new Pos(y, x),
          this
