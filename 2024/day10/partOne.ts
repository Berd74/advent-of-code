...
export async function partOne(path: string) {
  ...
  const playersByPeak: {[key: string]: Player[]} = {}

  playersOnTop.forEach(p => {
    const key = p.startPos.toString()
    const arr = playersByPeak[key] || []
    // ignore the players with the same start and end pos
    if (arr.find(curP => curP.pos.isEqual(p.pos))) {
      return
    }
    playersByPeak[key] = [...arr, p]
  })

  let sum = 0;
  for (const k in playersByPeak) {
    const players = playersByPeak[k]
    // console.log(players.map(p => ({startPos: p.startPos, pos: p.pos})));
    sum += players.length
  }
